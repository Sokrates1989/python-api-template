...
def get_service() -> WellnessService:
    """Get a database-aware wellness service instance."""
...
async def get_dashboard(current_user_id: str = Depends(get_user_id_from_token)):
...
async def list_activities(current_user_id: str = Depends(get_user_id_from_token)):
...
async def get_sync_bootstrap(
    ...
    """Return a combined wellness payload for local bootstrap/sync."""
    service = get_service()
...
async def get_sync_changes(
    ...
    """Return incremental wellness entity changes after an opaque timestamp cursor."""
    service = get_service()
...
async def update_activity(
    ...
    if request.favorite is None:
        raise HTTPException(
            status_code=400,
            detail="At least one mutable activity field must be provided",
        )
