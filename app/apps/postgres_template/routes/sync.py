"""Sync routes owned by the Postgres Template backend app."""
...
from apps.postgres_template.schemas.sync import (
...
from apps.postgres_template.services.sync_service import PostgresTemplateSyncService

router = APIRouter(tags=["sync"], prefix="/v1/sync")


def get_service() -> PostgresTemplateSyncService:
    """Return the Postgres Template sync service."""
    return PostgresTemplateSyncService()
...
async def pull_sync_changes(
    ...
    if entity_type and entity_type != PostgresTemplateSyncService.USER_PROFILE_ENTITY:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
