"""Sync routes owned by the template backend app."""
...
from apps.template_app.schemas.sync import (
...
from apps.template_app.services.sync_service import TemplateAppSyncService

router = APIRouter(tags=["sync"], prefix="/v1/sync")


def get_service() -> TemplateAppSyncService:
    """Return the template app sync service."""
    return TemplateAppSyncService()
...
async def pull_sync_changes(
    ...
    if entity_type and entity_type != TemplateAppSyncService.USER_PROFILE_ENTITY:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
