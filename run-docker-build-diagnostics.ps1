<#
.SYNOPSIS
Windows entry point for Docker/build diagnostics – delegates to Bash via WSL.
...
This script is a thin wrapper. It performs Windows-to-WSL handoff only:
  ...
  3. Forwards all arguments unchanged to run-docker-build-diagnostics.sh
     inside WSL.
  4. Propagates the Bash exit code as this script's exit code.

All runtime logic lives in run-docker-build-diagnostics.sh and the
tools/core-pdm-manager submodule Bash scripts.
Do not add runtime logic here.

.PARAMETER ScriptArgs
All arguments are forwarded verbatim to run-docker-build-diagnostics.sh.
#>
...
function Write-Status {
    ...
    Write-Host "[diagnostics] $Message" -ForegroundColor $Color
}
...
#
# Step 4 – Forward to run-docker-build-diagnostics.sh inside WSL.
#
Write-Status "Launching run-docker-build-diagnostics.sh in WSL..." "Cyan"
Write-Host ""
...
$bashCmd = 'cd "$WIN_REPO_PATH" && bash run-docker-build-diagnostics.sh ' + $bashArgString
