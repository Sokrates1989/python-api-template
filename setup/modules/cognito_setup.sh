...
# This module can be sourced by other bash scripts (setup.sh, quick-start.sh)
...
_cognito_setup_ensure_env_file() {
  ...
  if [ -f "${_template_file}" ]; then
    echo "📄 .env not found. Creating from template before configuring Cognito..."
    cp "${_template_file}" "${_env_file}"
    return 0
  fi

  echo "❌ Neither .env nor ${_template_file} exists. Cannot configure Cognito." >&2
  return 1
}
