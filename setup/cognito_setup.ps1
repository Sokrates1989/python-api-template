...
function Invoke-CognitoSetup {
    ...
    if (-not (Test-Path $script:CognitoEnvPath)) {
        if (-not (Test-Path $script:CognitoTemplatePath)) {
            Write-Host "[ERROR] Neither .env nor template exists. Cannot configure Cognito." -ForegroundColor Red
            return $false
        }
        Copy-Item $script:CognitoTemplatePath $script:CognitoEnvPath
